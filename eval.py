...
def run_evaluation(ground_truth_filename, proposal_filename, log_path,
                   max_avg_nr_proposals=100, 
                   tiou_thresholds=np.linspace(0.5, 0.95, 10),
                   subset='validation'):

    anet_proposal = ANETproposal(ground_truth_filename, proposal_filename, log_path,
                                 tiou_thresholds=tiou_thresholds,
                                 max_avg_nr_proposals=max_avg_nr_proposals,
                                 subset=subset, verbose=True, check_status=False)
    result_AUC = anet_proposal.evaluate()
    
    recall = anet_proposal.recall
    average_recall = anet_proposal.avg_recall
    average_nr_proposals = anet_proposal.proposals_per_video
    
    return (average_nr_proposals, average_recall, recall, result_AUC)
...
def evaluation_proposal(opt):
    ...
    uniform_average_nr_proposals_valid, uniform_average_recall_valid, uniform_recall_valid, result_AUC = run_evaluation(
    ...
    print_log(opt["log_path"], "AR@1 is \t",np.mean(uniform_recall_valid[:,0]))
    print_log(opt["log_path"], "AR@5 is \t",np.mean(uniform_recall_valid[:,4]))
    print_log(opt["log_path"], "AR@10 is \t",np.mean(uniform_recall_valid[:,9]))
    print_log(opt["log_path"], "AR@100 is \t",np.mean(uniform_recall_valid[:,-1]))
