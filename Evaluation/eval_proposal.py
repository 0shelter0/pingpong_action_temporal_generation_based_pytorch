...
class ANETproposal(object):
    ...
    def __init__(self, ground_truth_filename=None, proposal_filename=None, log_path=None,
        ...
        if self.verbose:
            print_log (self.log_path, '[INIT] Loaded annotations from {} subset.'.format(subset))
            nr_gt = len(self.ground_truth)
            print_log (self.log_path, '\tNumber of ground truth instances: {}'.format(nr_gt))
            nr_pred = len(self.proposal)
            print_log (self.log_path, '\tNumber of proposals: {}'.format(nr_pred))
            print_log (self.log_path, '\tFixed threshold for tiou score: {}'.format(self.tiou_thresholds))
    ...
    def evaluate(self):
        ...
        result_AUC = float(area_under_curve)/proposals_per_video[-1]

        if self.verbose:
            print_log (self.log_path, '[RESULTS] Performance on ActivityNet proposal task.')
            print_log (self.log_path, '\tArea Under the AR vs AN curve: {}%'.format(100.*float(area_under_curve)/proposals_per_video[-1]))
