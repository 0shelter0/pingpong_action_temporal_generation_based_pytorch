...
def dump_formatted_proposal(video_idx, video_id, num_frames, fps, gts,
    ...
    formatted_proposal_file.write(
        f'#{video_idx}\n{video_id}\n{num_frames}\n{1}\n{gts.shape[0]}\n')
    for gt in gts:
        formatted_proposal_file.write(f'{int(gt[2])} {int(np.floor(gt[0]*fps))} {int(np.ceil(gt[1]*fps))}\n')
    ...
    for i in range(proposals.shape[0]):
        ...
        if best_iou[i] == 0 and best_overlap[i] == 0:
            ...
            formatted_proposal_file.write(
                ...
                f'{int(np.floor(proposals[i][0]*fps))} {int(np.ceil(proposals[i][1]*fps))}\n')
...
def parse_args():
    ...
    parser.add_argument(
        '--ann-file',
        type=str,
        default='data/activitynet_annotations/anet_anno_val.json',
        help='name of annotation file')
    parser.add_argument(
        '--activity-index-file',
        type=str,
        default='data/activitynet_annotations/anet_activity_indexes_val.txt',
    ...
    parser.add_argument(
        ...
        type=str,
        default='output/anet_val_formatted_proposal.txt',
