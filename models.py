...
class BMN(nn.Module):
    def __init__(self, opt):
        ...
        self.att_layers = opt["att_layers"]
...
# ==============================xzq==========================
        self.p_conv = nn.Sequential(
            nn.Conv1d(self.feat_dim, self.d_model, kernel_size=1, padding=0),
            nn.ReLU(inplace=True)
        )

        #  d_model, seq_len, n_layers, d_k, d_v, d_ff, n_heads=4,
        self.encoder = Encoder(self.d_model, self.tscale, self.att_layers, d_k, d_v, d_ff, self.num_heads)

        # cry=====================cry==========================
        # self.fc1 = nn.Linear(self.feat_dim, self.hidden_dim_1d)
        # self.fc2 = nn.Linear(self.hidden_dim_1d, self.hidden_dim_1d)
        ...
        self.x_1d_s = nn.Sequential(
            nn.Conv1d(self.d_model, self.hidden_dim_1d, kernel_size=3, padding=1, groups=4),
            nn.ReLU(inplace=True),
        ...
        # ================new======= temporal fused   will neglect it
        self.x_1d_p = nn.Sequential(
            nn.Conv1d(self.d_model, self.hidden_dim_1d, kernel_size=3, padding=1),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x): #x[16,dim,seq_len]
        # ===========xzq==============
        x = self.p_conv(x)
        base_feature = self.encoder(x) #[16,512,100]

        # base_feature = self.x_1d_b(x)    #replaced with transformer
