...
def train_BMN(data_loader, model, optimizer, epoch, bm_mask, batch_size):
    model.train()
    epoch_pemreg_loss = 0
    epoch_pemclr_loss = 0
    epoch_tem_loss = 0
    ...
    for n_iter, (input_data, label_confidence, label_start, label_end) in enumerate(data_loader):
        ...
        confidence_map, start, end = model(input_data) # [16, 400, 100]
...
def test_BMN(data_loader, model, epoch, bm_mask):
    ...
    state = {'epoch': epoch + 1,
             'state_dict': model.state_dict()}
    torch.save(state, opt["checkpoint_path"] + "/BMN_checkpoint_xzq.pth.tar")
    if epoch_loss < best_loss:
        best_loss = epoch_loss
        torch.save(state, opt["checkpoint_path"] + "/BMN_best_xzq.pth.tar")
...
def BMN_Train(opt):
    ...
    for epoch in range(opt["train_epochs"]):
        ...
        train_BMN(train_loader, model, optimizer, epoch, bm_mask, opt["batch_size"])
...
def BMN_inference(opt):
    model = BMN(opt)
    model = torch.nn.DataParallel(model, device_ids=[0]).cuda()# checkpoint/BMN_9.pth
    checkpoint = torch.load(opt["checkpoint_path"] + "/BMN_9.pth") #checkpoint/BMN_AUC67.7.pth.tar
